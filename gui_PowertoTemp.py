...
def zth_step_from_cauer(t: np.ndarray, R_c: np.ndarray, C_c: np.ndarray) -> np.ndarray:
    # Simple numerical step response of series Cauer ladder: transform to equivalent Foster by using
    # if the repo has conversion prefer it; otherwise use a numeric approach: compute impulse response
    # by forming tau = R*C as approximation of first-order terms and summing like Foster (approx)
    # This is approximate; the repo's exact conversion should be used when available.
...
class ExtendedPowerTempApp(tk.Tk):
    ...
    def _build(self):
        ...
        ttk.Button(left, text='Export results CSV', command=self._on_export).pack(fill=tk.X)
        ...
        self.txt = tk.Text(right, height=6); self.txt.pack(fill=tk.X, pady=(6,0))
    ...
    def _on_compute(self):
        ...
        self._log(f'Duty (estimated): {duty:.6f}, Period: {period:.6g} s, Threshold: {thr:.6g} W')
        ...
        if (hR is not None) and (hC is not None):
            # append as an additional RC stage (simple model)
        ...
        # Prepare dense time grid covering several periods to show repetition — repeat power profile to cover a few cycles
        t_start, t_end = float(self.t_p[0]), float(self.t_p[-1])
        # Determine fundamental period (if found) else use data span
        rep_period = period if period>0 else (t_end - t_start)
        ncycles = 3  # show a few cycles
        total_T = rep_period * ncycles
        tgrid = np.linspace(0.0, total_T, 2000)

        # Build repeated power waveform by tiling P profile across cycles using modulo mapping
        # First create a function P_single(t) based on loaded points
        def P_single(tt):
            # map to original timespan via modulo
            span = t_end - t_start
            if span <= 0:
        ...
        # Temperature
        T = temp_from_power_and_zth(tgrid, P_on_grid, Zth_grid)
        ...
        # overlay foster reconstructed (if available)
        try:
        ...
        self.ax_temp.clear(); self.ax_temp.plot(tgrid, T, '-', label='ΔT (K)'); self.ax_temp.set_ylabel('ΔT (K)'); self.ax_temp.legend(); self.canvas_temp.draw()

        # Fill textual outputs
        self._log('--- Results ---')
        self._log(f'Duty = {duty:.6f} (estimated)')
        self._log(f'Foster R (N={len(self.Rf)}): ' + ', '.join([f'{x:.3g}' for x in self.Rf]))
        self._log(f'Foster C (N={len(self.Cf)}): ' + ', '.join([f'{x:.3g}' for x in self.Cf]))
        self._log(f'Cauer R (N={len(self.Rc)}): ' + ', '.join([f'{x:.3g}' for x in self.Rc]))
        self._log(f'Cauer C (N={len(self.Cc)}): ' + ', '.join([f'{x:.3g}' for x in self.Cc]))

        self._last = dict(tgrid=tgrid, P=P_on_grid, Zth=Zth_grid, T=T, duty=duty, Rf=self.Rf, Cf=self.Cf, Rc=self.Rc, Cc=self.Cc)
    ...
    def _on_export(self):
        ...
        if path.endswith('.npz'):
            ...
            # CSV export t, P, Zth, T
            import csv
            with open(path,'w',newline='') as f:
                w = csv.writer(f)
                w.writerow(['t_s','P_W','Zth_K_per_W','dT_K'])
                for ti,Pi,zi,Ti in zip(self._last['tgrid'], self._last['P'], self._last['Zth'], self._last['T']):
                    w.writerow([f'{ti:.9g}', f'{Pi:.9g}', f'{zi:.9g}', f'{Ti:.9g}'])
            messagebox.showinfo('Exported', f'CSV saved: {path}')
